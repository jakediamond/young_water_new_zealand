...
iso_fit_sinusoidal_model <- function(the.data, 
  ...
  site_model <- function(df) {
    ...
    fit.results <- robustbase::nlrob(formula = form,
                          data = df,
                          method = "M",
                          algorithm = "port",
                          ...
                          start = c(Amplitude = 1.0, 
                                    phase = 0,
                                    offset = offset.start.value),                         
                          lower = c(Amplitude = 0.1, phase = -3.1416, offset = -10), 
                          upper = c(5, 3.1416, -3),
                          tol = 1e-04, 
                          trace = FALSE)  
...
iso_return_base_nz_map <- function(location = "NZ") {
  ...
  nz.base <- ggmap::get_stamenmap(bbox, maptype = "terrain-background", zoom = 7)
